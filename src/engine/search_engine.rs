...
pub struct InMemorySearchEngine {
    documents: std::sync::Mutex<Vec<StructuredContent>>,
...
impl SearchEngine for InMemorySearchEngine {
    async fn index_one(&self, content: &StructuredContent) -> Result<()> {
        ...
        self.documents.lock().unwrap().push(content.clone());
        Ok(())
    }
    ...
    async fn index_batch(&self, items: &[StructuredContent]) -> Result<u64> {
        ...
        if let Some(ref embedder) = self.embedder {
            let vector_items: Vec<(String, String)> = valid
                .iter()
                .map(|c| {
            ...
            if !vector_items.is_empty() {
                let inputs: Vec<&str> = vector_items.iter().map(|(_, text)| text.as_str()).collect();
                let vectors = embedder.embed(&inputs)?;
                for ((id, _), vec) in vector_items.iter().zip(vectors) {
                    self.vectors.insert(id.clone(), vec);
                }
            }
        }
        self.documents.lock().unwrap().extend(valid);
        Ok(count)
    }
